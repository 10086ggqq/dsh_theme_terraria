...
**dsh_theme_terraria** is a Terraria-themed skin for [DeepSeek Harness](https://github.com/deepseek-ai/deepseek-harness) (dsh). It replaces the official React frontend with a single self-contained `index.html` (~76 KB, zero external JS dependencies), wrapping the "AI coding agent console" as a pixel adventure:
...
- **Single-file delivery**: the entire UI is inlined in one HTML file. The Vite build is essentially asset copying; there is no runtime framework.
...
```sh
# 1. Enter the DeepSeek Harness repo root
cd deepseek-harness

# 2. Install dependencies
pnpm install

# 3. Copy the theme files into the frontend directory
#    (contents of theme/ in this repository)
...
# 4. Build the theme (Vite only copies assets — finishes in seconds)
...
| Pitfall | Symptom | Fix |
|---|---|---|
| Browser caches old UI | Page still shows the old version after a rebuild | Hard refresh with `Ctrl+F5`, or append `?v=2` to the URL |
| Official frontend overwrites the build | Running the full `pnpm run build` re-bundles the official React UI into dist | Only run `pnpm --filter @deepseek-ai/dsh-web-frontend run build`; repeat steps 3–4 after any full-repo build |
| Key save fails | Toast reports "保存失败" | Make sure `pnpm dsh web` was started from your own terminal; sandboxed environments may block host-directory writes |
...
| WebSocket drops | Top bar shows "连接已断开，重连中…" | Normal auto-reconnect (2s interval); if the host process exited, restart `pnpm dsh web` |
...
- Primary: **Fusion Pixel 12px** (an open-source CJK pixel font), a single ~600 KB `woff2` bundled in public — crisp CJK rendering;
...
A faithful recreation of the game's main menu: logo, four pixel menu items (New Session / Continue / Settings / Credits), and the big tree silhouette over the forest background.

### Main Chat Interface (Three-Column Layout)

- **Top bar**: back button, session name (mode@workspace), connection lamp ("已连接到向导世界" — connected to the Guide's world), Chat/Terminal tab switch, sound toggle, settings entry;
...
- **API key**: password input, stored on the host via `credentials.set`; next to it, the tool-approval mode switch (auto-allow / manual confirm);
...
1. **Opening line**: every new session begins with — "欢迎来到泰拉瑞亚！我是你的向导。" (Welcome to Terraria! I am your Guide.)
...
7. **Credits egg**: the title-screen "制作人员" opens a slate reading — "泰拉瑞亚主题皮肤：向导陪你写代码。像素字体：Fusion Pixel 12px".
...
|---|---|
| **Zero-framework runtime** | The whole UI is vanilla DOM manipulation — no React/Vue runtime. The build output is one 76 KB HTML plus static assets |
...
| **Video via IndexedDB** | Live wallpapers (up to 512 MB) bypass the 5 MB localStorage cap; played through `URL.createObjectURL` with `revokeObjectURL` cleanup on switch |
...
```
git/                      <- this repository (GitHub-ready)
├── README.md             <- Chinese documentation
├── README_EN.md          <- English documentation (this file)
├── screenshots/          <- live screenshots (6)
...
└── theme/                <- theme files (copy into the dsh repo to apply)
    └── apps/web/
        ├── index.html        the entire theme source (HTML + CSS + JS in one file)
        ├── package.json      frontend package descriptor (references the original)
...
# (edit theme/apps/web/index.html, copy to apps/web/, then)
pnpm --filter @deepseek-ai/dsh-web-frontend run build

# Dev server with hot reload (when editing public assets)
...
The theme never touches any dsh backend code — it only replaces files under `apps/web/`, so uninstalling leaves zero residue. Browser-side personalization (wallpaper, sound toggle, approval preference) lives in localStorage / IndexedDB; clearing site data wipes it completely.
