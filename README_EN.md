...
```sh
# Install from the GitHub repository (replace your-owner with the actual owner)
dsh plugin --profile web add github:your-owner/dsh_theme_terraria
...
The theme mounts and unmounts wholesale through the plugin mechanism — it never modifies any file in the dsh installation, so uninstalling leaves zero residue. Browser-side personalization (wallpaper, sound toggle, approval preference) lives in localStorage / IndexedDB; clearing site data wipes it completely.
...
- Once listed, replace `your-owner` and `your-plugin-slug` in the badge link at the top with the corresponding parts of your listing page URL.
